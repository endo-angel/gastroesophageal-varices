...
def predict_file(model, model_input_size, file, result_path,
                 predict_area_num=1, min_area=None, points_gap=5, test_label_path=None, is_chk_dbscan=False):
    ...
    for item in ai_ctrs:
        ...
        if is_chk_dbscan:            
            for ii in range(sparse_ai_ctrs[0].shape[0]):
                dbscan_data_set.append([sparse_ai_ctrs[0][ii][0], sparse_ai_ctrs[0][ii][1]])

            clusterNum = dbscan_handle(dbscan_data_set, file_name, result_path, src_img_w, src_img_h)
    ...
    if test_label_path:
        return src_img_w, src_img_h, label_area, all_ai_area, iou, dice, clusterNum
    else:
        return src_img_w, src_img_h, all_ai_area, clusterNum
...
def predict_path(model, model_input_size, img_path, result_path, label_path, is_chk_dbscan=False):
    ...
    for file in test_files:
        src_img_w, src_img_h, label_area, ai_area, iou, dice, clusterNum = predict_file(model, model_input_size,
                                                                                        file, result_path,
        ...
        cluster_list.append(clusterNum)

    df = pd.DataFrame(
        {'filename': test_files, 'width': img_width_list, 'height': img_height_list, 'Ground Truth': label_area_list,
         'AI': ai_area_list, 'ai aera rate': ai_area_rate_list, 'IoU': iou_list, 'dice': dice_list, 'cluster': cluster_list})
...
if __name__ == '__main__':
    test_path = r'F:\gastroesophageal-varices\esophageal_seg_data\test_data'
    ...
    model_path = r'F:\gastroesophageal-varices\esophageal_seg_data\cache\X_21-07-22_09_54_17-0.999.hdf5'
